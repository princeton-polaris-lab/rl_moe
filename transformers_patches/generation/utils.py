...
class GenerationMixin(ContinuousMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        # Teacher-mixed sampling (MiniLLM): extract mix_in args before validation
        # These are NOT model forward kwargs — they control the sampling loop in _sample()
        _mix_in_model = model_kwargs.pop("mix_in_model", None)
        _mix_in_alpha = model_kwargs.pop("mix_in_alpha", None)

        self._validate_model_kwargs(model_kwargs.copy())
        self._validate_generation_mode(generation_mode, generation_config, generation_mode_kwargs)

        # Re-inject mix_in args for _sample() to pick up via **model_kwargs
        if _mix_in_model is not None:
            model_kwargs["mix_in_model"] = _mix_in_model
            model_kwargs["mix_in_alpha"] = _mix_in_alpha
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        # Teacher-mixed sampling (MiniLLM): mix student and teacher distributions
        mix_in_model = model_kwargs.pop("mix_in_model", None)
        mix_in_alpha = model_kwargs.pop("mix_in_alpha", None)
        ...
        # Teacher-mixed sampling: initialize separate KV cache kwargs for mix-in model
        m_outputs = None
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            if mix_in_model is not None and m_outputs is not None:
            ...
            # Teacher-mixed sampling: forward pass through mix-in model
            m_outputs = None
            if mix_in_model is not None:
                with torch.no_grad():
                ...
                if do_sample:
                    m_next_token_logits = m_outputs.logits[:, -1, :].to(copy=True, dtype=torch.float32, device=input_ids.device)
